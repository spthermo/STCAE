...
class VisdomVisualizer(object):
    ...
    def append_loss(self, epoch, global_iteration, loss, loss_name="total", mode='train'):
        ...
        if loss_name not in self.plots:
            self.plots[loss_name] = self.visualizer.line(X=numpy.array([global_iteration]), Y=numpy.array([loss]), opts=opts)
        else:
            self.visualizer.line(X=numpy.array([global_iteration]), Y=numpy.array([loss]), win=self.plots[loss_name], name=mode, update = 'append')
    ...
    def show_activations(self, maps, title):
        ...
        for i in range(c): #c
            ...
            heatmap = maps_cpu[i, :, :]
            heatmap_flipped = torch.flip(heatmap, [0, 1])
            self.visualizer.heatmap(heatmap_flipped,\
                opts=opts, win=self.name + title + "_window_" + str(i))

    def show_seg_map(self, maps, title, iter=0):
        h, w = maps.size()  
        maps_cpu = maps.detach().cpu()[:, :]
        opts = (
        {
            'title': title, 'colormap': 'Viridis'
        })
        heatmap_flipped = torch.flip(maps_cpu, [0, 1])
        self.visualizer.heatmap(heatmap_flipped,\
